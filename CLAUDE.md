...
- E2E tests (`crates/oxide-cli/src/e2e_{python,rust,typescript}_tests.rs` on `e2e_support.rs`, feature `e2e`) run real pipelines in containers across three ecosystems. Two tiers: hermetic tests are safe to gate on; canary tests hit live package registries and are `#[ignore]` so upstream outages can never turn the gate red. The engine does **not** pull images — the harness pre-pulls; see `ensure_image`.
