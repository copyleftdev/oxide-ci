...
impl PipelineRepository for PgPipelineRepository {
    async fn create(&self, definition: &PipelineDefinition) -> Result<Pipeline> {
        ...
        sqlx::query("INSERT INTO pipelines (id, name, description, definition, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)")
            .bind(id)
    ...
    async fn list(&self, limit: u32, offset: u32) -> Result<Vec<Pipeline>> {
        let rows = sqlx::query("SELECT id, name, definition, created_at, updated_at FROM pipelines ORDER BY created_at DESC LIMIT $1 OFFSET $2")
            .bind(limit as i64)
            .bind(offset as i64)
            .fetch_all(&self.pool)
            .await
    ...
    async fn update(&self, id: PipelineId, definition: &PipelineDefinition) -> Result<Pipeline> {
        ...
        let row = sqlx::query("UPDATE pipelines SET name = $2, description = $3, definition = $4, updated_at = $5 WHERE id = $1 RETURNING id, name, definition, created_at, updated_at")
            .bind(id.as_uuid())
            .bind(&definition.name)
            .bind(&definition.description)
            .bind(&definition_json)
