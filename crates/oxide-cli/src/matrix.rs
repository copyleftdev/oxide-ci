...
impl MatrixExpander {
    ...
    pub fn expand(&self, stage: &StageDefinition) -> Option<Vec<MatrixJob>> {
        ...
        let jobs: Vec<MatrixJob> = combinations
            ...
            .map(|(idx, vars)| {
                ...
                MatrixJob {
                    ...
                    index: idx + 1, // 1-based index for CLI
                    variables: vars,
                    display_name,
                }
            })
...
mod tests {
    ...
    fn test_matrix_expansion() {
        ...
        let jobs = expander.expand(&stage).unwrap();

        assert_eq!(jobs.len(), 6); // 2 OS × 3 versions
    }
