...
pub fn create_archive<W: Write>(
    ...
    match compression {
                ...
                .map_err(|e| oxide_core::Error::Internal(format!("Zstd init failed: {}", e)))?;
            {
                ...
                for p in paths {
                    ...
                    if abs_path.exists() {
                        ...
                        if abs_path.is_dir() {
                            builder.append_dir_all(name, &abs_path)
        ...
        _ => return Err(oxide_core::Error::Internal("Unsupported compression for archiving".into())),
    }
    Ok(())
}
...
pub fn extract_archive<R: Read>(
    ...
    match compression {
        ...
        _ => return Err(oxide_core::Error::Internal("Unsupported compression for extraction".into())),
    }
    Ok(())
}
