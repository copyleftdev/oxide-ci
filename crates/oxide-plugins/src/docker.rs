...
impl Plugin for DockerBuildPlugin {
    ...
    fn execute(&self, input: &PluginCallInput) -> Result<PluginCallOutput> {
        let dockerfile = input.params.get("dockerfile")
            .and_then(|v| v.as_str())
            .unwrap_or("Dockerfile");

        let context = input.params.get("context")
            .and_then(|v| v.as_str())
            .unwrap_or(".");
        ...
        info!("Building Docker image from {} in context {}", dockerfile, context);
        ...
        let status = cmd.status().map_err(|e| oxide_core::Error::Internal(format!("Failed to execute docker build: {}", e)))?;
