...
impl Plugin for GitCheckoutPlugin {
    ...
    fn execute(&self, input: &PluginCallInput) -> Result<PluginCallOutput> {
        let repo = input.params.get("repository")
            .and_then(|v| v.as_str())
            .ok_or_else(|| oxide_core::Error::Internal("Missing 'repository' input".into()))?;
            
        let ref_name = input.params.get("ref")
            .and_then(|v| v.as_str())
            .unwrap_or("main");
        ...
        let status = Command::new("git")
            ...
            .status()
            .map_err(|e| oxide_core::Error::Internal(format!("Failed to execute git clone: {}", e)))?;
        ...
        if ref_name != "main" && ref_name != "master" {
             let status = Command::new("git")
                ...
                .status()
                .map_err(|e| oxide_core::Error::Internal(format!("Failed to execute git checkout: {}", e)))?;
